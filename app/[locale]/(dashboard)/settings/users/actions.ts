...
export async function inviteUser(data: {
  ...
  try {
    // Every path in this action returns JSON `{ error }` or `{ success }`. If
    // anything unexpected escapes — e.g. the Supabase SDK throws on a 5xx,
    // or SUPABASE_SERVICE_ROLE_KEY is missing and createClient crashes — the
    // outer try/catch converts it to JSON so the RSC stream stays valid.
    try {
      ...
      // Send the invite email — this creates the auth.users row and fires
      // the handle_new_user trigger which inserts a profile row with the
      // same id. We stamp `needs_password_setup: true` into user_metadata
      // so the auth callback can route the user through /accept-invite.
      //
      // Wrap the SDK call defensively — GoTrue sometimes throws on 5xx rather
      // than returning `{ error }`, and we must never let that escape.
      let inviteData: { user: { id: string } | null } | null = null;
      let inviteErrorMessage: string | null = null;
      try {
        const res = await admin.auth.admin.inviteUserByEmail(data.email, {
          data: {
        ...
        inviteErrorMessage =
          sdkErr instanceof Error ? sdkErr.message : "Failed to send invite";
      }

      if (inviteErrorMessage || !inviteData?.user) {
        return { error: inviteErrorMessage ?? "Failed to send invite" };
      }

      // Stamp the newly-created profile with org_id, role(s), full_name.
      // Bypass RLS with the admin client. We set both `role` (primary) and
      // `roles[]` explicitly so they're guaranteed consistent even if the
      // sync trigger hasn't been applied yet.
      ...
      return toActionError(err, "inviteUser");
    }
  } catch (err) {
    if (isNextInternalThrow(err)) throw err;
    return toActionError(err, "inviteUser");
...
export async function updateUserRoles(
  ...
  try {
    ...
    if (isNextInternalThrow(err)) throw err;
    return toActionError(err, "updateUserRoles");
  }
}
...
export async function updateUserRole(
  ...
  try {
    ...
    if (isNextInternalThrow(err)) throw err;
    return toActionError(err, "updateUserRole");
  }
}
...
export async function updateUserProfile(
  ...
  try {
    ...
    if (isNextInternalThrow(err)) throw err;
    return toActionError(err, "updateUserProfile");
  }
}
...
export async function setUserPassword(
  ...
  try {
    ...
    // Supabase GoTrue admin SDK can throw on 5xx — wrap defensively.
    try {
      ...
      return {
        error:
          sdkErr instanceof Error
            ? `Password update failed: ${sdkErr.message}`
    ...
    if (isNextInternalThrow(err)) throw err;
    return toActionError(err, "setUserPassword");
  }
}
  ...
  try {
    ...
    try {
      ...
      return {
        error:
          sdkErr instanceof Error
            ? `Failed to resend invite: ${sdkErr.message}`
    ...
    if (isNextInternalThrow(err)) throw err;
    return toActionError(err, "resendInvite");
  }
}
